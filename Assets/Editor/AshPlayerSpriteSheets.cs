...
public static class AshPlayerSpriteSheets
{
    ...
    public static readonly CharacterSet AshKingPhase1 = new CharacterSet(
        ...
        new[]
        {
            ...
            //
            // 수정(2026-09-21, 전환 흐름 B "재의 알에서 깨어남") — 6장·8fps → <b>앞 2장·5fps</b>.
            // 예전에는 0.875초 안에 갑옷이 터지고 은발 여자가 서는 데(3~6번째 장)까지 다 보여준 뒤,
            // 재의 알을 거쳐 같은 모습이 한 번 더 나왔다 — 보스가 두 번 드러났다. 이제 무릎 꿇고
            // 갑옷에 금이 가는 데(1~2번째 장)까지만 보여주고, 여자는 알이 깨질 때 처음 드러난다.
            // 5fps = 한 장 0.2초(사용자 선택). 0.4초 뒤 두 번째 장에서 멈춰 있다가 0.5초의
            // 갑옷 붕괴 신호에 사라진다. 시트의 3~6번째 장은 그대로 남겨 둔다(지우지 않는다).
            new Sheet("ash-king-phase-transition", 6,
                new Segment("transition", 0, 2, 5, false)),
        });
    ...
    public static readonly CharacterSet AshKingPhase2 = new CharacterSet(
        ...
        new[]
        {
            ...
            // 추가 생성 — 2페이즈 전용 궁극기(재 폭발). 1페이즈에는 대응 모션이 없다.
            //
            // 수정(화면 전체 판정으로 변경): fps 10 → 6.
            //
            // 원래는 반경 5짜리 근접 패턴이라 <b>범위 밖으로 빠지는 것</b>이 회피였고, 0.3초
            // 예비동작이면 충분했다. 판정을 화면 전체로 키우면 그 회피가 성립하지 않는다.
            // 남는 수단은 <b>대시 무적 0.25초</b> 하나뿐인데, 0.3초 예비동작으로는 보고 나서
            // 대시를 누를 시간이 사실상 없다. 스태미나가 비어 있으면 아예 회피 불가다.
            //
            // 6fps면 6프레임 ÷ 6 = 1.0초, 폭발이 터지는 4번째 프레임이 <b>0.5초</b> 지점이다.
            // 대시 무적(0.25초)의 두 배라 보고 나서 누를 여유가 생긴다.
            //
            // 코드에서 대기만 늘리지 않고 클립을 늦춘 이유: 클립이 0.6초인데 코드가 1.0초를
            // 기다리면 마지막 프레임에서 0.4초 굳는다. 이 프로젝트가 내려찍기·파도에서 이미
            // 한 번씩 겪은 어긋남이라 같은 실수를 반복하지 않는다.
            //
            // 6fps는 이 세트의 idle(6fps)과 같은 속도다. 큰 동작이라 느려도 뚝뚝 끊겨 보이지 않는다.
            new Sheet("ash-king-phase2-ultimate", 6,
                new Segment("ultimate", 0, 6, 6, false)),
        });
