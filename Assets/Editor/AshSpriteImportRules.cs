...
public class AshSpriteImportRules : AssetPostprocessor
{
    ...
    /// <summary>
    /// 추가 생성 — 이 규칙의 판(version). 숫자를 올리면 유니티가 <b>대상 에셋을 전부 다시
    /// 임포트한다.</b>
    ///
    /// 이게 없으면 생기는 일: 임포트 파이프라인은 "에셋 파일이 바뀌었는가"만 본다.
    /// 이 스크립트를 고쳐도 png는 그대로이므로 유니티는 캐시된 임포트 결과를 그냥 재사용하고,
    /// <b>규칙을 바꿨는데 화면에도 메모리에도 아무 변화가 없다.</b> 폴더를 우클릭해
    /// Reimport를 누르면 그때야 먹는데, 그건 이 파일 맨 위 주석이 AssetPostprocessor를 고른
    /// 이유("누르는 걸 잊는 경로를 없앤다")와 정면으로 어긋난다.
    ///
    /// GetVersion은 유니티가 그 문제를 위해 제공하는 내장 훅이다. 직접 에디터 스크립트로
    /// 전체를 훑어 ImportAsset을 부르는 방법도 있지만, 그건 다시 "누르는 걸 잊는" 경로다.
    ///
    ...
    private void OnPreprocessTexture()
    {
        ...
        if (isCharacter)
        {
            ...
            //
            // <b>수정(텍스처 메모리 실측 후): Uncompressed → CompressedHQ.</b>
            //
            // 위 주석의 마지막 문장이 사실이 아니게 됐다. 씬·프리팹이 참조하는 텍스처를 전수로
            // 재보니 260MB 중 254MB가 무압축이었고, 그 절반인 <b>124MB가 플레이어 시트 10장</b>
            // 이다. 8방향으로 뽑으면서 한 장이 1536x2048(12MB), walk는 2048x2048(16MB)이 됐다.
            // "몇 장뿐"이라는 전제는 4방향 1536x256 시절의 이야기다.
            //
            // 그리고 앞 문장이 겨냥한 것은 <b>DXT5(BC3)</b>인데, CompressedHQ가 고르는 것은
            // <b>BC7</b>이다. 둘 다 1바이트/픽셀로 메모리는 같지만 BC7은 블록마다 쓸 수 있는
            // 색 엔드포인트와 분할 방식이 훨씬 많아서, BC3가 뭉개는 1~2px 대비가 살아남는다.
            // 즉 그때의 판단 근거가 이 설정에는 그대로 적용되지 않는다.
            //
            // <b>되돌리려면 이 한 줄만 Uncompressed로 되돌리면 된다.</b> .png.meta는 git이
            // 추적하므로 git checkout -- "Assets/Project/Art" 로 임포트 결과까지 원복된다.
            //
            // 주의 — BC 계열은 가로세로가 <b>4의 배수가 아니면 경고 없이 무압축으로 돌아간다.</b>
            // 인스펙터에는 "Compressed"로 보이기 때문에 눈으로는 안 잡힌다. 캐릭터 시트는
            // 전수 확인했다(1536x2048, 2048x2048, 1536x256, 1024x256 — 전부 통과).
            // 시트를 새로 뽑을 때 이 조건을 깨뜨리지 않아야 한다.
            importer.textureCompression = TextureImporterCompression.CompressedHQ;

            // 추가 생성 — BC7 인코딩 품질을 최대로 둔다.
            //
            // 기본값은 50이다. 이 값은 <b>런타임 성능이나 메모리와 무관하고</b> 임포트할 때
            // 인코더가 블록 분할을 얼마나 오래 탐색하는지만 정한다. 즉 비용은 임포트 시간
            // 한 번뿐인데 그 대가로 얻는 것이 정확히 우리가 걱정하는 부분(좁은 면적의 색 대비)
            // 이라, 여기서 아낄 이유가 없다.
