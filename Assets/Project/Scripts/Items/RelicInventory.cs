...
public class RelicInventory : MonoBehaviour
{
    ...
    ///
    /// 수정(겹친 주석 정리): 이 위에 "보관함에만 있어도 인정한다"는 설명이 하나 더 붙어 있었는데,
    /// <b>지금 동작과 반대라서 지웠다.</b> 그 설명은 열쇠가 일반 장착 칸을 쓰던 시절의 것이다.
    /// 그때는 장착을 요구하면 열쇠를 모을수록 유물 효과 없이 보스를 만나게 되니 보관함까지 세야 했다.
    /// 지금은 열쇠 전용 칸(<see cref="BossSlotCount"/>)이 따로 있어서 그 걱정이 구조적으로 없고,
    /// 코드도 칸만 센다. 틀린 설명을 남겨두면 코드보다 그쪽을 먼저 믿는 사람이 반드시 나온다.
    ///
    /// 세어둔 값을 들고 있지 않고 매번 세는 이유: 열쇠는 뺐다 꼈다 할 수 있다.
    /// 획득할 때 +1 하는 방식이면 빼는 경로마다 -1을 잊지 않아야 하는데, 그런 코드는
    /// 반드시 한 군데를 빠뜨린다. <b>칸이 곧 진실이므로 칸을 보고 센다.</b>
    /// </summary>
    ...
    public void Acquire(RelicData relic)
    {
        ...
        // 칸만 차지해서, 표식을 모을수록 오히려 약해진 채 보스를 만나게 된다.
        // 열쇠는 위에서 이미 전용 칸으로 빠졌지만, 열쇠가 아닌 표식이 생길 수 있어 이 판단은 남긴다.
        // (수정: 같은 설명이 두 번 겹쳐 있던 것을 하나로 합쳤다.)
